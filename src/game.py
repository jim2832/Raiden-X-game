...
player_img = pygame.image.load(os.path.join("img", "airforce.png")).convert()
...
pygame.mixer.music.set_volume(2) #設定音量
...
def draw_init():
    ...
    draw_text(screen, '雷電X', 64, WIDTH/2, HEIGHT/4)
    draw_text(screen, '操作說明：方向鍵移動飛船 按空白鍵發射子彈', 22, WIDTH/2, HEIGHT/2)
...
while running:
    ...
    for hit in hits: #若將一個石頭消滅，則會再補一顆石頭進來
        random.choice(expl_sounds).play() #隨機播放爆炸的聲音
